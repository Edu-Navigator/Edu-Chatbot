...
with DAG(
    ...
    with TaskGroup(group_id="location_digilearning") as tg1:
        ...
        task1_2 = table_full_refresh(
            task1_1, schema="raw_data", table="DIGITAL_LEARNING_LOCATION", conn_name="conn_production"
        )

    # 우리동네 디지털 안내소 : S3에 올라간 csv 데이터 추출
    with TaskGroup(group_id="extract_location_urdns") as tg2:
        task2_1 = et_urdn_location(
            csv_key="rawdata/csv_manual/urdn_digi_edu.csv", bucket="team7-batch", conn_name="aws_default"
        )
        task2_2 = table_full_refresh(
            task2_1, schema="raw_data", table="URDN_DIGITAL_LOCATION", conn_name="conn_production"
        )

    with TaskGroup(group_id="Transform_and_Load_location_urdns") as tg3:
        task_transform = get_details_location(
            schema="raw_data", table="URDN_DIGITAL_LOCATION", conn_name="conn_production"
        )

        task_load = table_full_refresh(
            task_transform, schema="ANALYTICS", table="URDN_DIGITAL", conn_name="conn_production"
        )
