    ...
    try:
        ...
        while True:
            try:
                ...
                wait.until(EC.visibility_of_element_located((By.XPATH, '//*[@id="container"]/div/div[3]/a[1]')))
            ...
            # 페이지 네비게이션 체크
            try:
                next_btn = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "#paging > div > a.btnNext")))
...
def upload_to_s3(image_list, bucket, base_key, batch_size=20, delay=5, conn_name="s3_conn", **context):
    ...
    for idx, img_info in enumerate(image_list, 1):
        try:
            ...
            s3client.put_object(
                Bucket=bucket, Key=save_key, Body=resized_bytes, ContentType="image/jpeg"
            ...
            failed_count += 1
            failed_images.append({"index": idx, "title": img_info.get("title"), "error": str(e)})
            continue
