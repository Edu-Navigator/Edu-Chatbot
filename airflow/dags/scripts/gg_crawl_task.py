...
def get_web_updated_date():
    url = "https://data.gg.go.kr/portal/data/service/selectServicePage.do?infId=Q318MFGTWD8D0Q36X8H112883673&infSeq=3"
    driver = None
    try:
        driver = get_driver()
        driver.get(url)
...
def get_max_created_at():
    ...
    max_created_str = max_created_at.astimezone().strftime("%Y-%m-%d")
...
def gg_crawl_task():
    ...
    try:
        ...
        for page in range(1, total_pages + 1):
            ...
            if len(res.get("ALifetimeLearningLecture", [])) > 1 and "row" in res["ALifetimeLearningLecture"][1]:
    ...
    use_cols = [
        ...
        "HMPG_ADDR",
        "OADT_SPORT_LECT_YN",
        "ACBS_EVALTN_PNT_RECOGN_YN",
        "LFLERNACCT_EVALTN_RECOGN_YN",
        "DATA_STD_DE",
    ]
